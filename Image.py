...
def median_blur(image: np.array):
    return cv.medianBlur(image, 5)


def dilate(image: np.array):
    kernel = np.ones((5, 5), np.uint8)
    return cv.dilate(image, kernel, iterations=1)


def erode(image: np.array):
    kernel = np.ones((5, 5), np.uint8)
    return cv.erode(image, kernel, iterations=1)


def morphology_ex(image: np.array):
    kernel = np.ones((5, 5), np.uint8)
...
def isolate_img(image: np.array, rgb_isolation: tuple[int, int, int], variation: int):
...
def get_only_white(image: np.array, variation: int = 50):
...
def get_only_black(image, variation: int = 100):
...
def reset_event_vars():
    global EVENT_DICT
    EVENT_DICT = {"i_event": 0, "next_i_event": 1, "i_display_images": 0, "arrays": [], "x": 0, "y": 0, "ocr": False,
...
def get_isolate_or_black_or_white(image: np.array, rgb_isolation: tuple[int, int, int], variation: int = -1):
...
def display_images_n_debug(images: list[np.ndarray],
    ...
    EVENT_DICT["ocr"] = ocr
    good_arrays = fill_images_array(images, rgb_isolation, variation)
    ...
    while not EVENT_DICT["exit_display"]:
        ...
        new_action = EVENT_DICT["i_event"] == EVENT_DICT["next_i_event"]
        if new_action:
            image = good_arrays[EVENT_DICT["i_display_images"]]
            if rgb_isolation is not None:
                image = isolate_img(image, rgb_isolation, 0 if variation == -1 else variation)
            if EVENT_DICT["ocr"]:
                ocr_text, image = ocr_image(image)
            EVENT_DICT["arrays"][EVENT_DICT["i_display_images"]] = image
            EVENT_DICT["next_i_event"] += 1
        plt.cla()
    plt.close()
    reset_event_vars()
...
if __name__ == '__main__':
    ...
    outputs = display_images_n_debug(images, rgb_isolation=(255, 255, 255))
