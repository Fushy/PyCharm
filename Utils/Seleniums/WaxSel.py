...
from FarmersWorldConf import PROJECT_NAME
...
def check_wax_approve(browser: Browser, pre_sleep: int = 1, refresh_min=None, ignore_new_approve=False) -> bool:
    def get_login_input(browser: Browser, fun) -> str | bool:
        ...
        connexion_input = browser.wait_element("", [connexion_input_xpath_1, connexion_input_xpath_2])
        if connexion_input:
            ...
            input_element = browser.get_element(ok_xpath)
            browser.element_click(input_element)
        browser.get_element_n_click(popup_close_xpaths)
        if not browser.wait_element("", messages_header_xpaths, leave=5000):
        ...
        if not verification_code:
            ...
            if account_bubble:
                ...
                sleep(1)
                browser.get_element_n_click(disconnect_xpath)
                sleep(2)
            ...
            while width and width < 1000 or browser.current_url() == WAX_APPROVE_URL:
                ...
                if approve_text == "Approve":
                    ...
                    if approve is not None and approve.is_enabled():
                        if ignore_new_approve:
                            alert(project_name + " " + browser.name + " ignore new approve", level=3)
                    ...
                    if red_login_button_text == "Approve" and not ignore_new_approve and red_login_button.is_enabled():
                        alert(project_name + " " + browser.name + " ignore new approve", level=3)
                    ...
                    while (red_login_button or white_login_empty_button or white_login_button) and width < 1000 \
                        ...
                        if white_login_button:
                            ...
                            def aux(browser, element):
                                browser.element_send(element, input_txt)

                            # alert(PROJECT_NAME + " " + browser.name + " have to approve a new connection", level=3)
                            ...
                            browser.element_send(auth_input, input_txt)
